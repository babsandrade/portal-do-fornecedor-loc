...
const UserDropdown = props => {
  ...
  function stringAvatar(user) {
    ...
    return {
      children: `${user?.toUpperCase()?.split(' ')[0][0]}`,
    }
  }
    ...
    <Fragment>
      ...
      >
        <Avatar
          ...
          onClick={handleDropdownOpen}
          sx={{ width: 38, height: 38, bgcolor: '#003566', color: 'white'}}
        />
      </Badge>
      <Menu
        anchorEl={anchorEl}
        ...
        <Box sx={{ py: 1.75, px: 6 }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            <Box sx={{ display: 'flex', ml: 2.5, alignItems: 'flex-start', flexDirection: 'column' }}>
              <Typography sx={{ fontWeight: 500 }}>{(user?.user_name)?.toUpperCase()}</Typography>
              <Typography variant='body2'>{(user?.role?.name)?.toUpperCase()}</Typography>
            </Box>
          </Box>
        </Box>
